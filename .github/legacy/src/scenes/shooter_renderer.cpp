...
void shooter_render(Scene* scene, Renderer& renderer) {
    if (!scene || !scene->user_data) return;

    ShooterData* data = static_cast<ShooterData*>(scene->user_data);
    ...
    for (int32_t i = 0; i < SHOOTER_MAX_PROJECTILES; i++) {
        const Projectile* p = &data->projectiles[i];
        if (!p->active) continue;
        float s = p->radius * 2.0f;
        renderer.draw_box(p->position, vec3_create(s, s, s), vec3_create(0.95f, 0.9f, 0.8f), 1.0f);
    }
